...
- Uploads pass through the authorised API with a streamed 25 MiB limit, timeout, actor-bound capacity reservation, MIME/extension/signature checks and create-only server Blob writes. No browser write SAS is issued. Read URLs expire after ten minutes. The free architecture does not include malware scanning; keep files download-only (except allow-listed inline images), train users not to open unexpected files, and review storage growth.
- Document quotas are 5 GiB and 5,000 records per organisation, plus three outstanding uploads per actor/organisation. They do not cap subscription costs, soft-deleted bytes, old untracked uploads, organisation creation or all API/email usage. Server-mediated uploads and conditional Cosmos guards add API/RU work within the existing resources; monitor throttling, transactions and metered storage/email usage.
- Production/Azure rejects `LOCAL_AUTH=true` and `DATA_BACKEND=memory`. Local identity bypass also requires explicit `NODE_ENV=development`. Authenticated mutations require JSON or the protected upload header so cross-site browser-simple requests are rejected; do not enable permissive credentialed CORS.
- Principal-header trust is valid only behind the configured Static Web Apps managed API. Do not expose the Functions code as an independent anonymous origin. Moving it to another topology requires trusted ingress or token signature/issuer/audience validation and a separate cost/security review.

## Upgrading upload protection

Deploy the API and browser together and refresh open clients. Previously issued write SAS URLs cannot be revoked by this code change; let the old ten-minute grants expire, or rotate the relevant Storage key during an approved containment procedure if exposure is suspected. Review historical orphan blobs and legacy duplicate document references privately. Source changes do not retroactively clean deployed data or revoke already-issued credentials.
