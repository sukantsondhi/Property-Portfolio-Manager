...
| Rental-year backup | An authorised Editor, Owner or Super admin starts a new rental year and closes the outgoing year. | Each active owner separately; structured record summary, a secure history link and eligible attachments up to 6 MiB raw total. Other files remain accessible through authenticated History. |
...
The app selects due items server-side and atomically claims hashed per-recipient stages in `_platform` before sending. Claim tokens protect completion; concurrent callers cannot send a claimed stage twice. Confirmed failed/cancelled sends become retryable. An unknown provider/network outcome remains suppressed as `uncertain`; stale `sending` claims also require operator review. The endpoint reports `delivery_review_required` rather than claiming that delivery is complete. Follow the [operations runbook](../operations.md) before changing claim status. These controls do not guarantee exactly-once delivery by an external email provider. Delivery records contain routing/status metadata, never email bodies or tenant notes.
